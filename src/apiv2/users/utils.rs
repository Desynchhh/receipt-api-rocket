...
pub fn validate_form_input(form: &Form<UserRegisterForm>) -> Vec<String> {
  ...
  if form.first_name.len() < 1 {
    errors.append(&mut vec![
      "You must enter your first name to create an account.".to_string(),
    ]);
  }
    
  if form.last_name.len() < 1 {
    errors.append(&mut vec![
      "You must enter your last name to create an account.".to_string(),
    ]);
  }
...
pub fn create_user_object<'a>(
  form: &'a Form<UserRegisterForm<'_>>,
  password: &'a str,
) -> NewUser<'a> {
  NewUser {
    email: form.email,
...
pub fn decode_jwt_str(jwt: &str) -> Result<TokenData<DecodedJwtUser>, jsonwebtoken::errors::Error> {
  ...
  decode::<DecodedJwtUser>(jwt, &DecodingKey::from_secret(secret.as_ref()), &validator)
}

...
use self::models::{ReceiptEntry, ErrorResponse, Receipt};
...
fn get_receipts() -> Result<Json<Vec<ReceiptEntry>>, Json<ErrorResponse>> {
    match utils::get_all_receipts() {
        None => Err(Json(ErrorResponse { error: "No receipts found.".to_string() })),
        Some(receipts) => {
            Ok(Json(receipts))
        }
    }
...
fn get_receipt(id: usize) -> Result<Json<ReceiptEntry>, Json<ErrorResponse>> {
    match utils::get_all_receipts() {
        ...
        Some(receipts) => {
            ...
            if receipt.is_none() {
                return Err(Json(ErrorResponse { error: format!("No receipt with id {} found.", id) }));
            }
    ...
    match utils::get_all_receipts() {
        ...
        Some(mut receipts) => {
            ...
            match utils::write_receipt_file(&receipts) {
                Err(e) => Err(Json(ErrorResponse { error: format!("{:?}", e) })),
...
#[put("/<receipt_id>", format="application/json", data="<edited_receipt>")]
fn update_receipt(receipt_id: usize, edited_receipt: Json<Receipt>) -> Result<Json<ReceiptEntry>, Json<ErrorResponse>> {
    match utils::get_all_receipts() {
        None => Err(Json(ErrorResponse{ error: "No receipts found.".to_string() })),
        Some(mut all_receipts) => {
            ...
            if receipt_option.is_none() {
                return Err(Json(ErrorResponse{ error: format!("No receipt with id {} found.", receipt_id)}));
            }
